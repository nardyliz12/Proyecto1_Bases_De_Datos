...
background = pygame.image.load('imagenes/fondo7.png')
...
def insert_votacion(connection, selected_option, username):
    try:
        with connection.cursor() as cursor:
            # Consulta para insertar en la base de datos
            sql = """
                INSERT INTO batalla (fecha, id_usuario, posion)
                VALUES (
                    NOW(),
                    (SELECT id_usuario FROM Usuarios WHERE BINARY nombre_usuario = %s),
                    %s
                )
            """
            cursor.execute(sql, (username, selected_option))
        connection.commit()
    except Exception as e:
        print(f"Error al insertar votación: {e}")

def obtener_ganador(connection):
    try:
        with connection.cursor() as cursor:
            # Consulta para obtener el ganador
            sql = """
                SELECT posion
                FROM (
                    SELECT posion, COUNT(*) AS votos
                    FROM batalla
                    WHERE DATE(fecha) = CURDATE()  -- Solo votaciones del día actual
                    AND fecha >= NOW() - INTERVAL 1 MINUTE  -- Votos de los últimos 60 segundos
                    GROUP BY posion
                ) AS subquery
                WHERE votos = (
                    SELECT MAX(votos)
            ...
            """
            cursor.execute(sql)
            ganador = cursor.fetchone()  # Devuelve el ganador (0 o 1)
            return ganador['posion'] if ganador else None
    except Exception as e:
        print(f"Error al obtener el ganador: {e}")
        return None

def insert_accion_batalla(connection, username, nombre_pokemon, nombre_ataque):
    try:
        with connection.cursor() as cursor:
            # Consulta para insertar en accion_batalla1
            sql = """
                INSERT INTO accion_batalla (fecha, id_usuario, id_pokemon, id_ataque)
                VALUES (
                    NOW(),
                    (SELECT id_usuario FROM Usuarios WHERE BINARY nombre_usuario = %s),
                    (SELECT id_pokemon FROM pokemon WHERE BINARY nombre_pokemon = %s),
                    (SELECT id_ataque FROM Ataque WHERE BINARY nombre_ataque = %s)
                )
            """
            cursor.execute(sql, (username, nombre_pokemon, nombre_ataque))
        connection.commit()
    except Exception as e:
        print(f"Error al insertar acción: {e}")


def obtener_ataque_mas_repetido(connection):
    try:
        with connection.cursor() as cursor:
            # Consulta para obtener el nombre del ataque más repetido
            sql = """
                SELECT A.nombre_ataque
                FROM accion_batalla AB
                JOIN Ataque A ON AB.id_ataque = A.id_ataque
                WHERE DATE(AB.fecha) = CURDATE()  -- Solo acciones del día actual
                AND AB.fecha >= NOW() - INTERVAL 1 MINUTE  -- Acciones de los últimos 60 segundos
                GROUP BY AB.id_ataque
                ORDER BY COUNT(*) DESC, RAND()
                LIMIT 1;
            """
            cursor.execute(sql)
            resultado = cursor.fetchone()
            return resultado['nombre_ataque'] if resultado else None
    except Exception as e:
        print(f"Error al obtener el ataque más repetido: {e}")
        return None
...
class Pokemon(pygame.sprite.Sprite):
    ...
    def draw_hp(self):
        hp_background = pygame.image.load('imagenes/barra2.png')  # Asegúrate de que esta ruta sea correcta
        ...
        if event.type == pygame.KEYDOWN:
            if game_status == 'player turn':
                ...
                if event.key == pygame.K_UP or event.key == pygame.K_LEFT:
                    ...
                    # 1. Insertamos la votación
                    insert_votacion(connection, selected_option, username)

                    # 2. Obtenemos el ganador antes de que ocurra la acción
                    ganador = obtener_ganador(connection)
                    if ganador is not None:
                        print(f"El ganador es: {ganador}")

                        # 3. Según el resultado del ganador, realizamos la acción
                        if ganador == 0:  # Opción Fight ganó
                            game_status = 'player move'  # Cambia a seleccionar movimiento
                        elif ganador == 1:  # Opción Potion ganó
                ...
                if event.key == pygame.K_UP or event.key == pygame.K_LEFT:
                    ...
                    # 1. Insertamos la acción en la base de datos
                    insert_accion_batalla(connection, username, player_pokemon.name, move.name)

                    # 2. Consultamos el ataque más repetido
                    ataque_mas_repetido = obtener_ataque_mas_repetido(connection)
                    if ataque_mas_repetido:
                        print(f"El ataque más repetido es: {ataque_mas_repetido}")

                        # 3. Buscamos el ataque en la lista de ataques del jugador para ejecutarlo
                        for atk in player_pokemon.moves:
                            if atk.name.lower() == ataque_mas_repetido.lower():
                                player_pokemon.perform_attack(rival_pokemon, atk)
                                break
            ...
            # Si estamos en el estado "player turn", movemos entre "Fight" y "Potion"
                ...
                if buttons["up"].collidepoint(mouse_pos) or buttons["left"].collidepoint(mouse_pos):
                    ...
                    if ganador is not None:
                        ...
                        # 3. Realizamos la acción según el resultado del ganador
                        if ganador == 0:
                            game_status = 'player move'  # Cambia a seleccionar movimiento
                        elif ganador == 1:
                            if player_pokemon.num_potions == 0:
